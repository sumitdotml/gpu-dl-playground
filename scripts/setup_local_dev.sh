...
if [[ "$OSTYPE" != "darwin"* ]]; then
    echo "⚠️  This script is designed for macOS. Proceeding anyway..."
fi
...
echo "⚠️  Note: Triton won't install on Apple Silicon, but that's expected"
...
if uv add triton 2>/dev/null; then
    ...
    echo "⚠️  Triton skipped (not available for Apple Silicon - will run remotely)"
fi
...
try:
    ...
    print('⚠️  Triton not available locally (expected on Apple Silicon)')
"
